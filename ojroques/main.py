#!/usr/bin/env python3
import logging
import ojroques.ot2 as ot
import ojroques.util
import ojroques.yao
...
class YaoGarbler(ABC):
    ...
    def __init__(self, circuits):
        circuits = ojroques.util.parse_json(circuits)
        self.name = circuits["name"]
        self.circuits = []

        for circuit in circuits["circuits"]:
            garbled_circuit = ojroques.yao.GarbledCircuit(circuit)
...
class LocalTest(YaoGarbler):
    ...
    def _print_evaluation(self, entry):
        ...
        for bits in [format(n, 'b').zfill(N) for n in range(2**N)]:
            ...
            result = ojroques.yao.evaluate(circuit, garbled_tables, pbits_out, a_inputs,
                                  b_inputs)
