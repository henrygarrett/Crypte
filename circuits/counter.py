...
"""
import math
number_of_elements = 8 # assumed greater than 1
count_size = math.ceil(math.log(number_of_elements,2)) + 1
print(count_size)
size = 32
import json
...
for i in range(number_of_elements - 1):
    for j in range(count_size):
        start1 = starter + i*((count_size-1)*5 + 2)
        if i == 0:
            if j == 0:
                circuit['gates'].append({"id": start1 + 1, "type": "XOR", "in": [inputs[0], inputs[1]]})
                circuit['gates'].append({"id": start1 + 2, "type": "AND", "in": [inputs[0], inputs[1]]})
                old_start = start1
            else:
                start = start1 + 2 + (j-1)*5
            ...
            if j == 0:
                circuit['gates'].append({"id": start1 + 1, "type": "XOR", "in": [old_start + 1, inputs[i+1]]})
                circuit['gates'].append({"id": start1 + 2, "type": "AND", "in": [old_start + 1, inputs[i+1]]})
                old_start = start1
            else:
                start = start1 + 2 + (j-1)*5
...
count = [start + i + 3 for i in range(0, -5*(count_size - 2) - 1, -5)]# assumes count_sizes is greater than 1 i.e 3 or more elements
