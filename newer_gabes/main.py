#!/usr/bin/env python3
import logging
import newer_gabes.ot2 as ot
import newer_gabes.util
import newer_gabes.yao
...
class YaoGarbler(ABC):
    ...
    def __init__(self, circuits):
        circuits = newer_gabes.util.parse_json(circuits)
        self.name = circuits["name"]
        self.circuits = []

        for circuit in circuits["circuits"]:
            garbled_circuit = newer_gabes.yao.GarbledCircuit(circuit)
...
class Alice(YaoGarbler):
    ...
    def __init__(self, circuits, oblivious_transfer=True):
        ...
        self.ot = ot.ObliviousTransfer(enabled=oblivious_transfer)
    ...
    def print(self, entry):
        ...
        for bits in [format(n, 'b').zfill(N) for n in range(2**N)]:
            ...
            print(*a_inputs)
            print(*b_keys)
            result = self.ot.garb1(a_inputs.values(), b_keys.values())
...
class LocalTest(YaoGarbler):
    ...
    def _print_evaluation(self, entry):
        ...
        for bits in [format(n, 'b').zfill(N) for n in range(2**N)]:
            ...
            result = newer_gabes.yao.evaluate(circuit, garbled_tables, pbits_out, a_inputs,
                                  b_inputs)
